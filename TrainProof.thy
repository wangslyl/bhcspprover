header {*The proof of the train on-board control system in Prover I*}
theory TrainProof
imports Train
begin
...
apply (simp add:mid11_def)
apply (rule conjR)
apply (simp add:b1_def Pre_def Ainv_def a_def)
apply fast 
apply (rule conjR) 
apply (simp add:b1_def t1_def equal_greater_def)
apply fast 
apply (simp add:b1_def Inv1_def) 
apply fast
done 

lemma mid22fact : "mid22 |- Ainv [&] (t1 [<] Real T1) [&] Inv1"
apply (simp add:mid22_def) 
apply (rule conjR)
apply (simp add:b1_def Pre_def Ainv_def a_def)
apply fast 
apply (rule conjR) 
...
apply (rule M1) 
apply (rule dbackwards_impR) 
apply (rule mid2fact2)
apply (rule thinL) 
apply (rule dbackwards_impR) 
apply (rule mid2fact4)
done
...
apply fast 
apply (rule conjR) 
apply (rule mid211fact3)
apply (rule conjR) 
apply (simp add:mid211_def b2_def afterb1_def b1_def Pre_def)
apply fast 
apply (simp add:mid211_def b2_def)
apply fast 
apply (simp add:Pre_def mid2_def) 
apply fast
done 
...
apply (rule conjR) 
apply (simp add:VAy_def v_def ya_def equal_greater_def)
apply fast  
apply (rule conjR) 
apply (rule conjL) 
...
apply (rule exchL) 
apply (rule thinL) 
apply (rule Breakfact11)
apply (simp add:Vinv_def equal_greater_def equal_less_def v_def) 
apply fast
apply (rule conjL) 
apply (rule thinL) 
apply (simp add:VAy_def)  
apply (rule conjL) 
apply (rule conjL)
...
defer apply (simp add:case11B_def) 
apply (rule impR) 
apply (rule disjR)
apply (rule disjL) 
apply fast+ 
...
apply (rule conjL) 
apply (rule thinL)
apply (simp add:v_def equal_greater_def equal_less_def) 
...
apply (simp add:equal_greater_def)
apply fast 
apply (simp add:equal_less_def) prefer 2
apply (simp add: equal_greater_def equal_less_def)
apply (rule conjR) 
apply (rule Trans1, auto)+
...
apply (rule thinR)  
apply (rule thinL) 
apply (simp add:case11B_def case12B_def case13B_def) 
apply (rule Transr3, auto) 
apply fast 
apply (rule disjL)
apply (rule conjL) 
apply (rule conjL) 
...
apply (simp add:mid211_def equal_greater_def) 
apply fast
apply (rule thinL) 
apply (rule mid2fact) 
apply (rule dimpR)
...
apply fast 
apply (rule thinL) 
apply (rule mid212fact) 
apply (rule Break1body)
done
...
apply (cut_tac qx = "Pre [&] case1B" and hx = "almostz Vinv" in IFelse, auto)
defer apply (simp add:case1B_def) 
apply fast 
apply (simp add:almostz_def)
apply fast 
...
apply (simp add:t2_def case1B_def) 
apply (rule impR) 
apply (rule conjR)+
apply (rule conjL) 
apply (rule exchL) 
...
apply (simp add:mid11_def equal_greater_def) 
apply fast
apply (rule thinL) 
apply (rule mid1fact) 
apply (rule dimpR)
